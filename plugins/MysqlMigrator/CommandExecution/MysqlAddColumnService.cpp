...
MysqlAddColumnService::MysqlAddColumnService()
{
}

const QString &MysqlAddColumnService::commandType() const
{
    return Commands::AddColumn::typeName();
}

bool MysqlAddColumnService::up(const Commands::ConstCommandPtr &command
...
bool MysqlAddColumnService::down(const Commands::ConstCommandPtr &command
                                 , CommandExecution::CommandExecutionContext &context) const
{
    QSharedPointer<const Commands::AddColumn> addColumn(command.staticCast<const Commands::AddColumn>());

    MysqlDropColumnService dropColumnService;
    return dropColumnService.up(Commands::CommandPtr(new Commands::DropColumn(addColumn->column().name()
                                                                              , addColumn->tableName()))
                                , context);
}

bool MysqlAddColumnService::isDownValid(const Commands::ConstCommandPtr &command
                                        , const CommandExecution::CommandExecutionContext &context) const
{
    QSharedPointer<const Commands::AddColumn> addColumn(command.staticCast<const Commands::AddColumn>());
